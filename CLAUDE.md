...
- Installs the MISSING (declared − installed) set via `brew safe-install --min-age 7` (age hold + CVE check + SHA verify; already-installed packages are never touched here — upgrades go through the daily gated `safe-upgrade`). After the held pass, anything still missing (held as too-new, or a live CVE) is offered for install with the freshness hold disabled (`--min-age 0`; CVE + SHA still enforced) via an interactive prompt — so a fresh machine is never left without a declared tool waiting on the age cushion. Non-interactive runs (no tty) skip the prompt and leave it held for the daily updater's reconcile pass. The age hold's job is to slow *upgrades*, not block *provisioning*.
